...
MOZ_ESR=
L10N_COMMIT="5889642a59bf99590ff704624ee4e0b172e6b361"
L10N_PV="$(ver_cut 1-2).7"
MOZ_PV=${PV}
MOZ_PV_SUFFIX=
SLOT="0"
WF_PN=Waterfox
...
SRC_URI="https://github.com/BrowserWorks/waterfox/archive/refs/tags/${PV}.tar.gz -> ${P}.gh.tar.gz
	${PATCH_URIS[@]}
	https://github.com/BrowserWorks/l10n/archive/${L10N_COMMIT}.zip -> ${WF_PN}-${L10N_PV}-l10n.zip
	wasm-sandbox? (
	...
	if [[ ${MERGE_TYPE} != binary ]] ; then
		...
		if use pgo ; then
			if ! has userpriv ${FEATURES} ; then
				eerror "Building ${WF_PN} with USE=pgo and FEATURES=-userpriv is not supported!"
			fi
...
src_prepare() {
	...
	# remove default optimize flag in .mozconfig
	#echo "ac_add_options --enable-bootstrap=no-update" >> "${S}"/.mozconfig
	# Create build dir
	BUILD_DIR="${WORKDIR}/${WF_PN}_build"
...
src_configure() {
	...
	export MOZILLA_FIVE_HOME="/usr/$(get_libdir)/${WF_PN}"
	...
	mozconfig_add_options_ac 'Waterfox' --with-app-name=Waterfox
...
src_install() {
	...
	for size in 22 24 256 ; do
		ln -s ${S}/waterfox/browser/branding/default${size}.png \
	...
	pax-mark m \
		...
		"${BUILD_DIR}"/dist/bin/${WF_PN} \
		"${BUILD_DIR}"/dist/bin/plugin-container

	DESTDIR="${D}" ./mach install || die

	# Upstream cannot ship symlink but we can (bmo#658850)
	#rm "${ED}${MOZILLA_FIVE_HOME}/${WF_PN}-bin" || die
	dosym ${WF_PN} ${MOZILLA_FIVE_HOME}/${WF_PN}-bin
...
#	dosym -r /usr/share/icons/hicolor/symbolic/apps/firefox-symbolic.svg /usr/share/icons/hicolor/symbolic/apps/org.mozilla.firefox-symbolic.svg
	...
	for icon in "${icon_srcdir}"/default*.png ; do
		...
		if [[ ${size} -eq 48 ]] ; then
			newicon "${icon}" ${WF_PN}.png
		fi

		newicon -s ${size} "${icon}" ${WF_PN}.png
	done

	# Install menu
	local app_name="${WF_PN}"
	local desktop_file="${FILESDIR}/icon/${WF_PN}-r3.desktop"
	local exec_command="${WF_PN}"
	local icon="${WF_PN}"
	local use_wayland="false"

	local desktop_filename="${WF_PN}.desktop"

	if use wayland ; then
		use_wayland="true"
	fi

	cp "${desktop_file}" "${WORKDIR}/${WF_PN}.desktop-template" || die

	sed -i \
		-e "s:@NAME@:${app_name}:" \
		-e "s:@EXEC@:${exec_command}:" \
		-e "s:@ICON@:${icon}:" \
		"${WORKDIR}/${WF_PN}.desktop-template" || die

	newmenu "${WORKDIR}/${WF_PN}.desktop-template" "${desktop_filename}"

	rm "${WORKDIR}/${WF_PN}.desktop-template" || die
...
#	fi
	...
	[[ -f "${ED}/usr/bin/${WF_PN}" ]] && rm "${ED}/usr/bin/${WF_PN}"
	newbin "${FILESDIR}/${WF_PN}-r1.sh" ${WF_PN}

	# Update wrapper
	sed -i \
		-e "s:@PREFIX@:${EPREFIX}/usr:" \
		-e "s:@DEFAULT_WAYLAND@:${use_wayland}:" \
		"${ED}/usr/bin/${WF_PN}" || die
