...
L10N_COMMIT='b2b530a96eaefc787ef3cedc0a9b61ec5e924232'
L10N_PV="$(ver_cut 1-2).9"
...
src_prepare() {
	...
	rm "${ff_patches}"/0021-bmo-1970223-python-3.14-support.patch || die
	rm "${ff_patches}"/0023-bmo-1962139-vendor-ffmpeg-8-headers.patch || die
	eapply "${ff_patches}"

	#eapply "${FILESDIR}/${PN}6.6.0-URLbar_unfuck.patch"
	#eapply "${FILESDIR}/${PN}-6.0-fix-langpack-id.patch"
	#if has_version '>=dev-libs/icu-78.1_pre' ; then
	#	eapply "${FILESDIR}/${PN}-6.6.4-icu-78.1.patch"
	#fi

	#if has_version '>=media-video/pipewire-1.5.80' ; then
	#	eapply "${FILESDIR}/firefox-143-pipewire-1.5-headers.patch"
	#fi
